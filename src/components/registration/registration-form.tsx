...
import { HOUSING_DATES_LABEL, HOUSING_RATES_USD, ROOM_BLOCK, totalAmountUsd } from '@/lib/pricing';
...
const TIERS = Object.keys(REGISTRATION_TIER_LABELS) as RegistrationFormValues['registration_type'][];
...
function scrollToFirstInvalidFormField(values: RegistrationFormValues): void {
  ...
  const run = () => {
    ...
    if (invalid) {
      invalid.scrollIntoView({ behavior: scrollBehavior, block: 'center', inline: 'nearest' });
    }
  };
...
function summarizeFieldErrors(errors: readonly unknown[] | undefined): string | undefined {
...
function registrationBlurFor(
  ...
  const onBlur = ({ fieldApi }: { fieldApi: { form: { state: { values: RegistrationFormValues } } } }) =>
    registrationFieldMessage(key, fieldApi.form.state.values);

  return deps?.length ? { onBlur, onChangeListenTo: [...deps] } : { onBlur };
}

function FieldFeedback({ meta, id }: { meta: { errors?: readonly unknown[] }; id?: string }) {
    ...
    <AnimatePresence mode="sync" initial={false}>
      {msg ? (
        <motion.p
          ...
          transition={{ duration: reduced ? 0 : UI_MS_SHORT / 1000, ease: 'easeOut' }}
...
function SummaryUsd({ amount, className, suffix = '' }: { amount: number; className?: string; suffix?: string }) {
  const reduced = useReducedMotion() ?? false;
  return (
    <motion.span
...
function HousingRoomRateCard({ code }: { code: keyof typeof HOUSING_RATES_USD }) {
  ...
  const rows: { label: string; value: number }[] = [
    ...
    { label: 'Per guest — full 3-night stay (sharing)', value: r.fullStaySharedGuest },
  ];
  ...
  return (
    <div className="overflow-hidden rounded-xl border border-stone-200/85 bg-white shadow-[0_1px_3px_rgba(15,23,42,0.05)] ring-1 ring-black/[0.03]">
      <div className="border-b border-emerald-100/90 bg-gradient-to-br from-emerald-50/95 via-white to-teal-50/30 px-4 py-3.5">
        <p className="font-sans text-[10px] font-semibold uppercase tracking-[0.16em] text-emerald-800/90">Rate plan</p>
        <p className="mt-1 font-sans text-base font-semibold tracking-tight text-stone-900">Room Type {code}</p>
      </div>
...
export function RegistrationForm() {
  ...
  const progressPercent =
    SECTIONS.length > 0 ? Math.min(100, ((resolvedIndex + 1) / SECTIONS.length) * 100) : 0;
  ...
  useEffect(() => {
    ...
    if (!nav) return;
    const btn = nav.querySelector<HTMLButtonElement>(`[data-section-nav="${activeSection}"]`);
    if (!btn) return;
  ...
  useEffect(() => {
    const elements = SECTIONS.map((s) => document.getElementById(`section-${s.id}`)).filter(
    ...
    onSubmit: async ({ value }) => {
      ...
      try {
        ...
        if (saved.status === 409) {
          setFormMessage({
            type: 'error',
            text: registerJson.error ?? 'You have already registered with this email address.',
          });
          setStatus('idle');
          return;
        }
    ...
    <div className="relative mx-auto grid max-w-6xl gap-8 px-4 pb-24 pt-6 md:px-6 lg:grid-cols-[minmax(0,1fr)_320px]">
      <div className="space-y-8 md:space-y-10">
        ...
        >
          <div className="mb-3">
            <div className="min-w-0 flex-1">
              <p className="text-[10px] font-semibold uppercase tracking-[0.18em] text-stone-500">Registration</p>
          ...
          <div
            ...
            aria-valuemin={0}
            aria-valuemax={100}
            aria-valuetext={SECTIONS[resolvedIndex]?.title ?? 'Registration progress'}
              ...
              return (
                <motion.button
                  ...
                  className={cn(
                    ...
                    past && 'border-emerald-200/90 bg-emerald-50/80 text-emerald-900 hover:bg-emerald-50',
                    current &&
                      'border-emerald-600 bg-white text-emerald-950 shadow-[0_4px_14px_-4px_rgba(5,150,105,0.35)] ring-2 ring-emerald-600/20',
                    !past && !current && 'border-stone-100/90 bg-white/70 text-stone-500 hover:bg-white hover:text-stone-600',
                  )}
                >
                  <span
                    className={cn(
                      ...
                      !past && !current && 'bg-stone-100 text-stone-500 ring-1 ring-stone-200/80',
                    )}
                    aria-hidden
                  >
                    {past ? '✓' : current ? '\u25CF' : '\u25CB'}
                  </span>
                  <span className={cn('line-clamp-2 text-[10px] font-semibold leading-snug', !current && !past && 'opacity-90')}>
                    {s.title}
                  </span>
                </motion.button>
              );
        ...
        >
          ...
          <Section id="personal" title="Personal Information" subtitle="Tell us how to reach you.">
            <div className="grid gap-4 md:grid-cols-12">
              <div className="md:col-span-4">
                <Label required>First name</Label>
                <form.Field name="first_name" validators={registrationBlurFor('first_name')}>
                  {(field) => <Input field={field} />}
                </form.Field>
              </div>
              <div className="md:col-span-2">
                <Label>Middle initial</Label>
                <form.Field name="middle_initial" validators={registrationBlurFor('middle_initial')}>
                  {(field) => <Input field={field} maxLength={8} />}
                </form.Field>
              </div>
              <div className="md:col-span-6">
                <Label required>Last name</Label>
                <form.Field name="last_name" validators={registrationBlurFor('last_name')}>
                  {(field) => <Input field={field} />}
                </form.Field>
              </div>
              <div className="md:col-span-6">
                <Label required>Email address</Label>
                <form.Field name="email" validators={registrationBlurFor('email')}>
                  {(field) => <Input field={field} type="email" autoComplete="email" />}
                </form.Field>
              </div>
              <div className="md:col-span-6">
                <Label required>Phone number</Label>
                <form.Field name="phone" validators={registrationBlurFor('phone')}>
                  {(field) => <Input field={field} type="tel" autoComplete="tel" />}
                </form.Field>
              </div>
            </div>
          </Section>

          <Section id="professional" title="Professional Background" subtitle="Your role and training.">
            <div className="grid gap-4">
              <div>
                <Label required>Professional role</Label>
                <form.Field name="professional_role" validators={registrationBlurFor('professional_role')}>
                  {(field) => {
                    ...
                    return (
                      <>
                        <FormSelect
                          ...
                          onChange={(e) =>
                            field.handleChange(e.target.value as RegistrationFormValues['professional_role'])
                          }
                          aria-invalid={errMsg ? true : undefined}
                          aria-describedby={errMsg ? registrationFeedbackId('professional_role') : undefined}
                        >
                        ...
                        <FieldFeedback meta={field.state.meta} id={registrationFeedbackId('professional_role')} />
                      </>
                    );
                  }}
                </form.Field>
                  ...
                  role === 'other' ? (
                    <div>
                      ...
                      <form.Field
                        ...
                        validators={registrationBlurFor('professional_role_other', ['professional_role'])}
                      >
                        {(field) => <Input field={field} />}
                      </form.Field>
                    </div>
              ...
              <div>
                ...
                <form.Field name="highest_degree" validators={registrationBlurFor('highest_degree')}>
                  {(field) => <Input field={field} />}
                </form.Field>
              </div>
              <div className="grid gap-4 md:grid-cols-2">
                <div>
                  ...
                  <form.Field name="institution" validators={registrationBlurFor('institution')}>
                    {(field) => <Input field={field} />}
                  </form.Field>
                </div>
                <div>
                  <Label>Department / unit</Label>
                  <form.Field name="department" validators={registrationBlurFor('department')}>
                    {(field) => <Input field={field} />}
                  </form.Field>
                </div>
              </div>
              <div className="flex items-center gap-3 rounded-lg border border-stone-200 bg-white px-3 py-2">
                <form.Field name="is_student" validators={registrationBlurFor('is_student', ['registration_type'])}>
                  {(field) => {
                    const errMsg = summarizeFieldErrors(field.state.meta.errors);
                    return (
                      <>
                        <label className="flex cursor-pointer items-center gap-2 text-sm text-stone-800">
                          <input
                            ...
                            aria-invalid={errMsg ? true : undefined}
                            aria-describedby={errMsg ? registrationFeedbackId('is_student') : undefined}
                            className="size-4 rounded border-stone-300 text-emerald-600 focus:ring-emerald-500"
                          />
                          Are you a student?
                        </label>
                        <FieldFeedback meta={field.state.meta} id={registrationFeedbackId('is_student')} />
                      </>
                    );
                  }}
                </form.Field>
          ...
          <Section id="location" title="Location" subtitle="Where you are joining from.">
            <div className="grid gap-4 md:grid-cols-3">
              <div>
                <Label required>Country</Label>
                <form.Field name="country" validators={registrationBlurFor('country')}>
                  {(field) => <Input field={field} autoComplete="country-name" />}
                </form.Field>
              </div>
              <div>
                <Label required>State / province / region</Label>
                <form.Field name="state_region" validators={registrationBlurFor('state_region')}>
                  {(field) => <Input field={field} />}
                </form.Field>
              </div>
              <div>
                <Label required>City</Label>
                <form.Field name="city" validators={registrationBlurFor('city')}>
                  {(field) => <Input field={field} />}
                </form.Field>
              </div>
            </div>
          ...
          <Section id="preferences" title="Conference Preferences" subtitle="Accessibility and meals.">
            <div className="grid gap-4">
              <div>
                <Label required>Dietary requirements</Label>
                <form.Field name="dietary_requirements" validators={registrationBlurFor('dietary_requirements')}>
                  {(field) => {
                    const errMsg = summarizeFieldErrors(field.state.meta.errors);
                    return (
                      <>
                        <FormSelect
                          ...
                          onBlur={field.handleBlur}
                          onChange={(e) =>
                            field.handleChange(e.target.value as RegistrationFormValues['dietary_requirements'])
                          }
                          aria-invalid={errMsg ? true : undefined}
                          aria-describedby={errMsg ? registrationFeedbackId('dietary_requirements') : undefined}
                        >
                        ...
                        <FieldFeedback meta={field.state.meta} id={registrationFeedbackId('dietary_requirements')} />
                      </>
                    );
                  }}
                </form.Field>
                ...
                {(d) =>
                  d === 'other' ? (
                    <div>
                      ...
                      <form.Field name="dietary_other" validators={registrationBlurFor('dietary_other', ['dietary_requirements'])}>
                        {(field) => <Input field={field} />}
                      </form.Field>
                    </div>
                  ) : null
              ...
              <div>
                ...
                <form.Field name="accessibility_needs" validators={registrationBlurFor('accessibility_needs')}>
                  {(field) => {
                    const errMsg = summarizeFieldErrors(field.state.meta.errors);
                    return (
                      <>
                        <FormSelect
                          ...
                          onBlur={field.handleBlur}
                          onChange={(e) =>
                            field.handleChange(e.target.value as RegistrationFormValues['accessibility_needs'])
                          }
                          aria-invalid={errMsg ? true : undefined}
                          aria-describedby={errMsg ? registrationFeedbackId('accessibility_needs') : undefined}
                        >
                        ...
                        <FieldFeedback meta={field.state.meta} id={registrationFeedbackId('accessibility_needs')} />
                      </>
                    );
                  }}
                </form.Field>
                  ...
                  a === 'other' ? (
                    <div>
                      ...
                      <form.Field
                        ...
                        validators={registrationBlurFor('accessibility_other', ['accessibility_needs'])}
                      >
                        {(field) => <Input field={field} />}
                      </form.Field>
                    </div>
              ...
              <div>
                ...
                <form.Field name="additional_notes" validators={registrationBlurFor('additional_notes')}>
                  {(field) => {
                    const errMsg = summarizeFieldErrors(field.state.meta.errors);
                    return (
                      <>
                        <textarea
                          ...
                          rows={3}
                          aria-invalid={errMsg ? true : undefined}
                          aria-describedby={errMsg ? registrationFeedbackId('additional_notes') : undefined}
                          className={cn(
                        ...
                        />
                        <FieldFeedback meta={field.state.meta} id={registrationFeedbackId('additional_notes')} />
                      </>
                    );
                  }}
                </form.Field>
          ...
          <Section id="housing" title="Housing" subtitle="Optional room block at the host hotel.">
            <div className="overflow-hidden rounded-2xl border border-emerald-200/65 bg-white text-sm text-stone-800 shadow-[0_10px_40px_-14px_rgba(15,80,65,0.18)] ring-1 ring-black/[0.04]">
              ...
              <div className="space-y-6 p-5 md:p-7">
                ...
                <blockquote className="rounded-xl border border-stone-200/90 bg-stone-50/80 px-4 py-3 text-sm leading-relaxed text-stone-700 md:px-5">
                  Rooms are first-come, first-served for <strong> {HOUSING_DATES_LABEL} </strong> (3 nights). Studio suites include
                  1&nbsp;King bed and a sofa bed (up to 2 adults).
                </blockquote>
            ...
            <div className="mt-4 space-y-3">
              ...
              <form.Field name="needs_housing" validators={registrationBlurFor('needs_housing')}>
                {(field) => {
                  const errMsg = summarizeFieldErrors(field.state.meta.errors);
                  return (
                    <>
                      <div
                        ...
                        className={cn('flex flex-wrap gap-4 rounded-lg text-sm', errMsg && 'outline outline-red-400/70')}
                      >
                      ...
                      <FieldFeedback meta={field.state.meta} id={registrationFeedbackId('needs_housing')} />
                    </>
                  );
                }}
              </form.Field>
                  ...
                  h === 'yes' ? (
                    <div className="grid gap-4 md:grid-cols-2">
                      <div>
                        ...
                        <form.Field name="room_type" validators={registrationBlurFor('room_type', ['needs_housing'])}>
                          {(rf) => {
                            const errMsg = summarizeFieldErrors(rf.state.meta.errors);
                            return (
                              <>
                                <div
                                  id={registrationControlId('room_type')}
                                  className={cn('space-y-2', errMsg && 'rounded-lg outline outline-red-400/70')}
                                >
                                  {(['A', 'B'] as const).map((room) => (
                                    <label key={room} className="flex cursor-pointer items-center gap-2 text-sm">
                                      <input
                                        type="radio"
                                ...
                                <FieldFeedback meta={rf.state.meta} id={registrationFeedbackId('room_type')} />
                              </>
                            );
                          }}
                        </form.Field>
                      ...
                      <div>
                        ...
                        <form.Field
                          name="occupancy_type"
                          validators={registrationBlurFor('occupancy_type', ['needs_housing'])}
                        >
                          {(of) => {
                            const errMsg = summarizeFieldErrors(of.state.meta.errors);
                            return (
                              <>
                                <div
                                ...
                                <FieldFeedback meta={of.state.meta} id={registrationFeedbackId('occupancy_type')} />
                              </>
                            );
                          }}
                        </form.Field>
          ...
          <Section id="heard" title="How Did You Hear About Us" subtitle="Select all that apply.">
            <Label required className="mb-3 block">
              How did you hear about A-DNA Global Conference USA 2026?
            </Label>
            <form.Field name="heard_about_us" validators={registrationBlurFor('heard_about_us')}>
              {(field) => (
                <>
                  <div
                    ...
                    className={cn(
                      ...
                      summarizeFieldErrors(field.state.meta.errors) && 'rounded-xl outline outline-red-400/70',
                    )}
                  >
                          ...
                          <input
                            ...
                            onChange={() => {
                              const next = checked
                                ...
                                : [...list, option];
                              field.handleChange(next as RegistrationFormValues['heard_about_us']);
                            }}
                  ...
                  <FieldFeedback meta={field.state.meta} id={registrationFeedbackId('heard_about_us')} />
                </>
              )}
            </form.Field>
            <form.Subscribe selector={(s) => s.values.heard_about_us.includes('other')}>
              {(show) =>
                show ? (
                  <div className="mt-3">
                    ...
                    <form.Field
                      ...
                      validators={registrationBlurFor('heard_about_other', ['heard_about_us'])}
                    >
                      {(field) => <Input field={field} />}
                    </form.Field>
                  </div>
          ...
          <Section id="social" title="Social Media" subtitle="Optional, helps us celebrate you online.">
            <div className="grid gap-4 md:grid-cols-2">
              <div>
                <Label>Instagram handle</Label>
                <form.Field name="instagram_handle" validators={registrationBlurFor('instagram_handle')}>
                  {(field) => <Input field={field} placeholder="@yourhandle" />}
                </form.Field>
              </div>
              <div>
                <Label>X handle</Label>
                <form.Field name="x_handle" validators={registrationBlurFor('x_handle')}>
                  {(field) => <Input field={field} placeholder="@yourhandle" />}
                </form.Field>
              </div>
              <div>
                <Label>LinkedIn URL</Label>
                <form.Field name="linkedin_url" validators={registrationBlurFor('linkedin_url')}>
                  {(field) => <Input field={field} type="url" placeholder="https://www.linkedin.com/in/..." />}
                </form.Field>
              </div>
              <div>
                <Label>Facebook name / handle</Label>
                <form.Field name="facebook_handle" validators={registrationBlurFor('facebook_handle')}>
                  {(field) => <Input field={field} />}
                </form.Field>
              </div>
              <div className="md:col-span-2">
                <Label>Other platform &amp; handle</Label>
                <form.Field name="other_social" validators={registrationBlurFor('other_social')}>
                  {(field) => <Input field={field} />}
                </form.Field>
              </div>
            </div>
          ...
          <Section id="payment" title="Payment" subtitle="Choose your registration bundle.">
            <fieldset id={registrationControlId('registration_type')} className="space-y-3">
              <Label required className="mb-2 block">
                Registration type
              </Label>
              <form.Field name="registration_type" validators={registrationBlurFor('registration_type', ['is_student'])}>
                {(field) => (
                  <>
                        ...
                        return (
                          <label
                            ...
                            className={cn(
                              ...
                              field.state.value === key ? 'border-emerald-600 ring-1 ring-emerald-500' : 'border-stone-200',
                            )}
                          >
                            <input
                              type="radio"
                            ...
                            />
                            <span>
                              <span className="block font-semibold text-stone-900">{meta.label}</span>
                            </span>
                          </label>
                        );
                      })}
                    </div>
                    <FieldFeedback meta={field.state.meta} id={registrationFeedbackId('registration_type')} />
                  </>
                )}
              </form.Field>
            </fieldset>
            ...
            <div className="mt-8 flex flex-col gap-3 border-t border-stone-200 pt-6 sm:flex-row sm:items-center sm:justify-between">
              <div className="space-y-2 text-sm text-stone-600">
                <p>You will securely pay with Paystack in the next step.</p>
                <p className="text-xs leading-relaxed text-stone-500">
                  A-DNA is a 501(c)(4) nonprofit. Amounts paid are not tax-deductible.
                </p>
              </div>
              <motion.button
                type="submit"
                disabled={submitting}
                whileHover={submitting || motionUi.reduced ? undefined : { scale: 1.02 }}
                whileTap={submitting || motionUi.reduced ? undefined : { scale: 0.98 }}
                transition={motionUi.micro}
      ...
      <aside className="lg:sticky lg:top-6 lg:self-start">
        <form.Subscribe selector={(state) => state.values}>
          {(vals) => {
            ...
            const housingLine =
              vals.needs_housing === 'yes' && vals.room_type && vals.occupancy_type
                ? `Housing (${vals.room_type}, ${vals.occupancy_type})`
                : 'Housing';

            return (
              <LayoutGroup>
                <motion.div layout className="rounded-2xl border border-stone-200 bg-white p-5 shadow-sm">
                  <p className="text-xs font-semibold uppercase tracking-[0.12em] text-stone-500">Summary</p>
                  <div className="mt-4 space-y-3 text-sm text-stone-800">
                    <div className="flex justify-between gap-3">
                      <span>Conference registration</span>
                      <SummaryUsd amount={t.registrationAmount} className="font-semibold" />
                    </div>
                    <AnimatePresence initial={false}>
                      {vals.needs_housing === 'yes' ? (
                        ...
                        >
                          ...
                          <SummaryUsd amount={t.housingAmount} className="font-semibold" />
                        </motion.div>
                      ) : null}
                    </AnimatePresence>
                    <motion.div
                  ...
                  <p className="mt-4 text-[11px] leading-relaxed text-stone-500">
                    Totals reflect current published rates for August 2026. Charges are finalized at Paystack checkout.
                  </p>
                  <p className="mt-2 text-[11px] leading-relaxed text-stone-500">
                    A-DNA is a 501(c)(4) nonprofit; payments are not tax-deductible.
                  </p>
                </motion.div>
              </LayoutGroup>
            );
        ...
        <div className="mt-6 rounded-2xl border border-emerald-100 bg-emerald-50/70 p-4 text-xs text-emerald-950 shadow-sm lg:hidden">
          ...
          <p className="mt-1 leading-relaxed">Email info@g-dna.org for accessibility and payment questions.</p>
        </div>
      </aside>
            ...
            return (
              <div className="mx-auto flex max-w-6xl items-center justify-between gap-3 px-2">
                ...
                <motion.button
                  type="button"
                  disabled={submitting}
                  whileHover={submitting || motionUi.reduced ? undefined : { scale: 1.03 }}
                  whileTap={submitting || motionUi.reduced ? undefined : { scale: 0.97 }}
                  transition={motionUi.micro}
                  onClick={() => {
                    document.getElementById('section-payment')?.scrollIntoView({ behavior: 'smooth' });
                    window.setTimeout(() => {
                      const fm = document.getElementById('conference-registration-form');
        ...
        <div className="p-6 md:p-8">
          <header className="border-b border-stone-100/90 pb-5">
            <div className="min-w-0 space-y-2">
              <h2 className="font-sans text-lg font-semibold leading-snug tracking-tight text-stone-900 md:text-xl">{title}</h2>
              {subtitle ? <p className="max-w-[52ch] text-sm leading-relaxed text-stone-500">{subtitle}</p> : null}
            </div>
          </header>
...
function Label({ children, required, className }: { children: React.ReactNode; required?: boolean; className?: string }) {
  return (
    <label className={cn('mb-1 block text-xs font-semibold uppercase tracking-wide text-stone-600', className)}>
      {children}
...
function dietaryLabel(v: RegistrationFormValues['dietary_requirements']): string {
...
function accessibilityLabel(v: RegistrationFormValues['accessibility_needs']): string {
